...
Where dimensions include "testing", "security", "code_quality".
"""
...
def publish_code_quality_evidence_for_story(story_id: str) -> Tuple[bool, str]:
    """
    Quality badge in MissionLog uses evidenceDim "code_quality". :contentReference[oaicite:2]{index=2}
    Repo truth is produced by tools/run_story_lint.py in evidence/lint/<story>.json. :contentReference[oaicite:3]{index=3}
    """
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Extract MissionLog evidence (testing + security + quality).")
    g = parser.add_mutually_exclusive_group(required=True)
    g.add_argument("--story", help="Extract only this story ID (e.g., ST-05).")
    g.add_argument(
        "--all",
        action="store_true",
        help="Extract all stories that have evidence (testing/security/quality).",
    )
